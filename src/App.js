  ...
  return (
    <div className="bg-[#2789D8] text-white font-inter min-h-screen flex flex-col items-center justify-center px-4">
      <div className="text-center mt-10 md:mt-20">
        <h1 className="text-2xl md:text-6xl font-bold mb-2">Get letter grades in Altitude Learning</h1>
        <p className="text-sm md:text-lg">for students of The Mount Vernon School</p>
      </div>
      ...
      <div className="my-10 w-full flex justify-center items-center">
        <div className="hover:bg-gray-300 rounded-full p-2 inline-flex items-center justify-center transition duration-300 mx-2">
          <button onClick={handlePrev} className="text-xl md:text-2xl">&laquo;</button>
        </div>
        <img src={process.env.PUBLIC_URL + '/images/' + images[currentImageIndex]} alt="Carousel" className="rounded border-4 border-white max-w-[60vw] md:max-w-[30vw] max-h-[30vw]" />
        <div className="hover:bg-gray-300 rounded-full p-2 inline-flex items-center justify-center transition duration-300 mx-2">
          <button onClick={handleNext} className="text-xl md:text-2xl">&raquo;</button>
        </div>
      </div>
      ...
      <div className="flex justify-center space-x-2 mb-16">
        {images.map((image, index) => (
          <img key={index} src={process.env.PUBLIC_URL + '/images/' + image} alt="Preview" onClick={() => handleImageClick(index)} className="w-10 h-auto cursor-pointer" />
        ))}
      </div>

      {/* Explanation */}
      <p className="text-center text-sm md:text-lg mb-4">Click on the button below to be taken to the Chrome Web Store to install:</p>

      <div className="flex flex-col items-center mb-4">
        {/* Install Button */}
        <a href="https://chromewebstore.google.com/detail/mount-vernon-school-altit/npejnoaeodlhhllfjdnddllhlegapdeg?hl=en">
          <img src={process.env.PUBLIC_URL + '/images/webstore_button.png'} alt="Install on Chrome Web Store" className="mt-4 hover:opacity-80 transition duration-200" style={{ width: '150px', height: 'auto', borderRadius: '10px' }} />
        </a>
      ...
      <footer className="mt-auto py-4 flex flex-col justify-center items-center">
        <p className="text-xs mb-2 text-center">because you shouldn't have to use a chart to see your grade</p>
